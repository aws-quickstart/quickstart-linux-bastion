...
To customize the banner, create a ASCII text file with your own banner content,
...
The log includes a history of the commands executed when a user logs in. See for example the log shown in <<logging>>. It records that a user logged in through a specific IP
address, attempted to remove the password file as a standard user, and then escalated
...
recommend that you enable the bastion host banner, as described in link:#_enabling_and_customizing_the_linux_bastion_host_banner[Enabling and Customizing the Linux bastion host banner].
The default banner text includes the alert shown in <<banner>>, which you can customize.

The `bastion.log` file has the immutable attribute set, and cannot be easily deleted or tampered
with. However, in the event it is deleted by a malicious actor, there is a shadow file containing a copy of `bastion.log` located in `/var/log/audit/audit.log`. 
As this shadow file is a copy that itself can be deleted, the Quick Start also
stores the contents of `bastion.log` remotely using the CloudWatch Logs service. The
...
* Inbound access to the bastion hosts are locked down to known CIDR scopes. This is
...
We recommend that you follow these best practices when using the architecture built
by the Quick Start:

* When you add new instances to the VPC that require management access from the
bastion host, associate a security group inbound rule with each instance. The rule should reference the
bastion security group as the source. It is also important to limit access to the required ports for administration.

* During deployment, the public key from the selected Amazon EC2 key pair is associated
with the user `ec2-user` in the Linux instance. For additional users, create
users with the required permissions and associate them with their individual authorized
public keys for SSH connectivity.

* For the bastion host instances, select the number and type of instances
...
filter and isolate log messages from individual bastion hosts more easily. Every
